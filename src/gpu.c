...
COLOUR gpu_framebuffer[160 * 144]; 
unsigned char gpu_tiles[384][8][8];

SDL_Event gpu_sdl_event;

const COLOUR gpu_palette[4] = {
    { 255, 255, 255 },
    { 192, 192, 192 },
    { 96, 96, 96 },
    { 0, 0, 0 },
...
{
	SDL_UpdateTexture(gpu_texture, NULL, gpu_framebuffer, GAMEBOY_WIDTH * sizeof(unsigned char) * 3);
...
void gpu_render_tiles()
{
	...
	for (int pixel = 0; pixel < 160; pixel++)
	{
		...
		COLOUR pixel_colour;
		pixel_colour = gpu_background_palette[colour_id];

		gpu_framebuffer[pixel_offset].r = pixel_colour.r;
		gpu_framebuffer[pixel_offset].g = pixel_colour.g;
		gpu_framebuffer[pixel_offset].b = pixel_colour.b;
		pixel_offset++;
	}
}
	...
	for (int spritenum = 0; spritenum < 40; spritenum++)
	{
		...
		if ((gpu.scanline >= y_pos) && (gpu.scanline < (y_pos + y_size)))
		{
			...
			for (int tile_pixel = 7; tile_pixel >= 0; tile_pixel--)
			{
				...
				unsigned short colour_address = CHECK_BIT(attributes, 4) ? 0xFF49 : 0xFF48;
				int colour_id = gpu_get_colour_id(colour_num1, colour_num2);
				...
				COLOUR pixel_colour;
				if (colour_address == 0xFF48)
				{
					pixel_colour = gpu_sprite_palette[0][colour_id];
				}
				else if (colour_address == 0xFF49)
				{
					pixel_colour = gpu_sprite_palette[1][colour_id];
				}

				int x_pixel = 0 - tile_pixel;
				x_pixel += 7;

				gpu_framebuffer[pixel_offset + x_pixel].r = pixel_colour.r;
				gpu_framebuffer[pixel_offset + x_pixel].g = pixel_colour.g;
				gpu_framebuffer[pixel_offset + x_pixel].b = pixel_colour.b;
			}
		}
	}
}
